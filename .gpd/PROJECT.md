...
- [ ] **(B matter-sourcing, re-scoped MATTER-ON-FLAT by Phase 70.1)** Does matter M in V_1/V_{1/2} SOURCE the perturbation h of g = eta + h on the flat KKT background, through the cubic-norm cross-terms 2Re((x2 x1)x3) (cross-term on/off test)? The M=0 baseline is flat KKT Minkowski (DERIVED, not pure-Lambda). -- v17.0
- [ ] **(C Einstein structure, re-scoped by Phase 70.1)** Does the matter-sourced perturbation h satisfy the LINEARIZED EINSTEIN equation box h ~ kappa T on the flat KKT eta background (correct tensor + gauge structure)? This CAN FAIL (honest negative if h obeys a non-Einstein PDE) -- the real gate; NO Lambda=0 tripwire (flatness is DERIVED from KKT, not inserted). -- v17.0
...
_Last updated: 2026-05-31 after Phase 70.1 (Revise A0 — physical-metric selection). VERDICT (human-ratified): the physical spacetime metric is the η+h bridge (KKT Minkowski η + matter perturbation h), NOT the cone-Hessian curvature; the "cone-Hessian IS the metric" thesis is FALSIFIED (its M=0 vacuum is the non-Einstein R_time×H³ {0,−1,−1,−1}; spin-2 on a fixed non-Einstein background is non-gauge). The route is RESTATED, not abandoned: the cone-Hessian is the matter-source structure (cross-terms source h), spacetime is the flat KKT slice; the flat M=0 vacuum is DERIVED from KKT det_2 (no inserted Λ / no vacuum circularity tripwire). Phases 70.1 + 71 (SURVIVES) complete; Phase 70 superseded; Phase 72 UNBLOCKED — re-plan as matter-on-flat; Phase 73 re-scoped to the linearized-Einstein can-fail test (□h̄ ~ κT on flat η). v16.0 PROVED (RING) and archived; v14.0 paused pending JMP referee report; v15.0 complete & archived. Next: re-plan Phase 72 (matter-on-flat). Original v17.0 launch 2026-05-30: a fresh physics-side route to gravity as the intrinsic curvature the V_0 slice inherits from the symmetric-cone metric g_X = Hess(−log det), replacing the abandoned lattice/Fisher route and the circular det/GST/Weinberg supergravity route._
