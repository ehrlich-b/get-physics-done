# Research Roadmap: v18.0 — Gravity as the Curvature of the Peirce-Frame (Cartan/MacDowell-Mansouri) Connection on h_3(O) (Experiential Measure on Structure Space)

**Archived:** 2026-06-03
**Status:** SHIPPED — decisive result. Phases 74–78 (8 plans), all verified HIGH, human-ratified at every gate. The route SURVIVED the Phase-75 coframe-reduction KILL gate ((E_11,u) forces a 4d Lorentzian (1,3) coframe carrying SO(3,1)), the Phase-77 flatness sub-gate (`R[ω]≠0` for `M≠0`), and matter-sourcing — then the strongest claims returned **NEGATIVE on two independent axes**: Phase 77 (B, dynamical) `G[g]` is NOT Einstein-form `κT+Λg` for any single global `(κ,Λ)`; Phase 78 (C, tensor/action) the MM ε-contraction is **NOT forced** by the h_3(O) trace form `Tr(X∘Y)`/cubic norm `det_3` (`det_3≡0` on the soldered Lorentz block; ε only via the orientation-ambiguous volume form; bare invariant 4-form space already dim=2). **Combined verdict: `fp-imported-action`** — a curved, matter-sourced, position-dependent Lorentzian slice carrying a forced SO(3,1) coframe, but NOT Einstein gravity without a posited action (same honest-partial class as Singh/Castro/GST; the h_3(O) exceptional-structure escape hatch is CLOSED). The Berry curvature `Im(QGT)` was reinterpreted (Phase 76) as the internal `so(6)=SU(4)` gauge sector, NOT gravity — a deferred SM-unification bonus. INDEPENDENT of v17.0's Ph73 NONE (different tensor: antisymmetric/Lie `R[ω]` vs symmetric/real cone-Hessian — does NOT bind). Exact over Q on every decisive verdict; `octonion_algebra.py` BANNED. NEGATIVE-RESULT-IS-SUCCESS.

For current project status, see `.gpd/ROADMAP.md`.

## Overview

This milestone computed the **antisymmetric/Lie-sector** gravitational object every prior route in the program ignored: the curvature 2-form `F = dA + A∧A` of the Peirce-frame Cartan/MacDowell-Mansouri connection `A = ω⊕e` on h_3(O), equivalently the **Berry curvature = imaginary part of the quantum geometric tensor (QGT)** of the rank-1 primitive-idempotent state family. The dead v17.0 cone-Hessian route computed the QGT's **real part** (Fubini-Study metric) and returned NONE; Provost-Vallée (1980) is the load-bearing justification that NONE binds only `Re(Q)`, leaving the imaginary part — a genuinely different tensor — untouched. The question was decided by cheap KILL gates first: Phase A (coframe reduction, SURVIVED) and Phase A.5 (Berry curvature, retired as a gate after it was found to test the internal gauge sector), gating the expensive full Cartan assembly (Phase B) and the forced-vs-posited circularity audit (Phase C). Everything decisive is **exact over Q** (the Berry sector over Q(i), Re/Im split); the det SSOT is `code/ring_lemma_verification.py` det_3. **Scope = GR** (the Lorentz Spin(3,1) block) — NOT the Standard Model. NEGATIVE-RESULT-IS-SUCCESS: a clean KILL/SOFT-KILL is a full publishable closure; the realized outcome `fp-imported-action` (forced coframe, imported action) is an honest partial, reported at true strength.
