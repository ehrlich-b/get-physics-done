# Requirements Archive: v15.0 The P5 <-> Basin Restriction Lemma
...
- [x] **DERV-62-01**: Set up the Peirce/bottleneck conditional expectation E: h_3(O) -> h_3(C_u) explicitly (Effros-Stormer): positive, unital, range = JB-subalgebra. State exactly what E preserves -- in particular whether it is coherent with the sequential product a&b = sqrt(a) b sqrt(a), not only the Jordan product. (Satisfied 2026-05-24: E = entrywise C_u-projection, u=e_7; positive, unital, idempotent, E|_A=id; Jordan-product-preserving ON THE SLICE but NOT a Jordan morphism on the ambient. Decisive crux framed as AMBIENT E-transport residual R.)
- [x] **DERV-62-02**: Determine whether the self-modeling data on A (its V_BM and its sequential product) is INDUCED by / consistent with the ambient h_3(O) Jordan structure under E -- demonstrated on the actual non-associative structure, NOT asserted. Treat the sequential product (CFC square roots in the non-associative ambient) explicitly. (Satisfied 2026-05-24: DEMONSTRATED on generic ambient X,Y -- exact residual R != 0 computed in h_3(O), associator 524/9 load-bearing on the same triple. NOT asserted.)
- [x] **DERV-62-03**: If preservation holds, state and prove the RESTRICTION embedding lemma. If it fails, characterize the obstruction precisely: name the exact structure clause (iii) needs that E cannot induce from the non-associative ambient, and what minimal extra input would be required. (Satisfied 2026-05-24: OBSTRUCTION (O) -- characterized precisely: ||R||^2 = 38593/72, R_11 = -2, defect in C_u across the three E_11 Peirce grades; minimal extra input = Hanche-Olsen induced-vs-imported, NOT required by coexistence-as-island. Refines RESTRICTION, not a collapse.)
- [x] **VALD-62-01**: SymPy/matrix verification on h_3(O): construct E onto h_3(C_u) for a fixed u in S^6 (e.g. u = e_7), and test that sqrt(a) b sqrt(a) for a, b in the slice -- computed in the ambient h_3(O) -- lands back in the slice (preservation), or exhibit explicit leakage / a non-associativity obstruction. (Satisfied 2026-05-24: exact-SymPy assert harness `tests/test_embedding_under_E.py` exit 0, verdict (O), is_zero_exact=[False,False]; slice-internal control R=0 (test can yield P, non-rigged). Two routes agree.)

### Phase 63 -- Verdict

- [x] **DERV-63-01**: Assemble `derivations/p5-basin-restriction/RESULT.md`: either a clean RESTRICTION theorem (statement + proof, touching the actual non-associative structure) or a precisely-characterized obstruction, with the explicit consequence for the program (through-line real vs C and O independent posits) and what would have to change to close an obstruction. (Satisfied 2026-05-24: RESULT.md FINALIZED -- CHARACTERIZED OBSTRUCTION refining RESTRICTION to coexistence-as-island; program consequence = through-line SURVIVES as the island through-line; Hanche-Olsen minimal-extra-input stated.)
- [x] **DERV-63-02**: Adversarial fresh-eyes review of the verdict before RESULT.md finalizes, checking the three reward-hacking guards (no redefining clause iii; no conflating the two composites; no asserted preservation without demonstration on h_3(O)) and confirming a negative result was reported honestly rather than a positive forced. (Satisfied 2026-05-24: separate-wave review, 3/3 guards PASS against the real Phase 60/61/62 artifacts; harness re-run exit 0 verdict (O); honest-negative confirmed; committed c879fd74 BEFORE finalization 93615617.)

### Milestone-Level Deliverables

- [x] **DERV-00-01**: `derivations/p5-basin-restriction/attempt-NN.md` -- one per serious proof/obstruction attempt (numbered, most recent last); each states inputs used, the argument, and failure mode if any. (Satisfied 2026-05-24: attempt-01..05 contiguous, coverage-audited in attempt-05.md.)
- [x] **DERV-00-02**: Final verdict line in RESULT.md: RESTRICTION THEOREM (through-line) or CHARACTERIZED OBSTRUCTION (independent posits), with the one-sentence consequence for the Radical Relativity program. (Satisfied 2026-05-24: verdict line = CHARACTERIZED OBSTRUCTION refining RESTRICTION to coexistence-as-island; through-line SURVIVES -- NOT independent posits, NOT a program collapse. DRAFT marker removed.)
...
| DERV-62-01 | 62 | Satisfied (E set up; crux framed as ambient transport) |
| DERV-62-02 | 62 | Satisfied (demonstrated on generic ambient X,Y; R != 0) |
| DERV-62-03 | 62 | Satisfied (OBSTRUCTION (O) characterized precisely) |
| VALD-62-01 | 62 | Satisfied (exact-SymPy harness; verdict (O)) |
| DERV-63-01 | 63 | Satisfied (RESULT.md finalized; CHARACTERIZED OBSTRUCTION) |
| DERV-63-02 | 63 | Satisfied (adversarial review, 3/3 guards PASS) |
| DERV-00-01 | 60-63 | Satisfied (attempt-01..05 contiguous) |
| DERV-00-02 | 63 | Satisfied (verdict line; coexistence-as-island) |
...
**Milestone outcome (2026-05-24):** All 16 primary requirements SATISFIED. Verdict (DERV-00-02) = **CHARACTERIZED OBSTRUCTION** refining RESTRICTION to coexistence-as-island — the "self-modeling -> QM -> h_3(O)" through-line SURVIVES (observer = self-contained C* island self-certifying M_3(C)^sa QM on the slice A = h_3(C_u) = range E; E is the access/projection map, NOT a Jordan/SP morphism on the non-associative ambient). A clean, honestly-reported obstruction is the contract-sanctioned outcome (NEGATIVE-RESULT-IS-SUCCESS). FUTR-01 (Paper 7 LaTeX) NOT triggered (outcome was not a clean positive RESTRICTION theorem).

_Requirements defined: 2026-05-23_
_Last updated: 2026-05-24 at v15.0 milestone completion (all requirements satisfied; verdict CHARACTERIZED OBSTRUCTION)_
