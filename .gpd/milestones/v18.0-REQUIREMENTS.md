# Requirements Archive: v18.0 — Gravity as the Curvature of the Peirce-Frame (Cartan/MacDowell-Mansouri) Connection on h_3(O)

**Archived:** 2026-06-03
**Status:** SHIPPED — all 15 requirements (DERV ×4, CALC ×5, VALD ×6) complete; all 4 contract claims resolved. The route SURVIVED the Phase-75 coframe-reduction KILL gate and the Phase-77 flatness sub-gate, then returned a two-axis NEGATIVE on the strongest claims: the assembled `F=dA+A∧A` Lorentz block IS the genuine 4d Riemann tensor (independent Levi-Civita cross-check exact over Q, torsion=0) of a curved, matter-sourced, position-dependent Lorentzian slice, but (Phase 77, B) `G[g]` is NOT Einstein-form `κT+Λg` for any single global `(κ,Λ)` vs an AST-guarded independent `T[M]`, and (Phase 78, C) the MM ε-contraction is **NOT forced** by the h_3(O) trace form/cubic norm (`det_3≡0` on the soldered Lorentz block; ε only via the orientation-ambiguous volume form; bare invariant 4-form space already dim=2). **Final verdict: `fp-imported-action`** — a forced SO(3,1) coframe but an imported action (honest partial, NOT a derivation; same class as Singh/Castro/GST). The Phase-76 Berry-curvature SOFT-KILL was OVERTURNED (it tested the internal `so(6)=SU(4)` gauge sector, not gravity — a deferred SM bonus). Exact over Q throughout; `octonion_algebra.py` BANNED. NEGATIVE-RESULT-IS-SUCCESS.
...
- [x] **DERV-01**: Re-pass the det SSOT (`code/ring_lemma_verification.py` det_3) — Cayley-Hamilton + 324/324 inner-derivation annihilation = dim f_4 = 52; confirm `octonion_algebra.py` is NOT imported on any decisive path.
- [x] **DERV-02**: Verify EXACTLY over Q the load-bearing tangent identity `E_11∘δ = (1/2)δ` for a basis of `δ ∈ V_{1/2}(E_11)`, and that the tangent space to the primitive-idempotent variety at `E_11` is exactly `V_{1/2}(E_11)` (16-dim). _(Peirce-under-E_11 via `bulk_geometry_verification.py:peirce_indices_under_E11` / `embedding_under_E_verification.py` — `peirce_coupling.py` does not exist.)_
- [x] **VALD-01**: Reproduce the calibration anchors via `code/orbit_dimension_gate.py` (single-copy dim 24 / Spin(8)=28 / trdeg 3; e_6=78=52+26; orbit(E_11)=17; Stab_{E_6}(E_11)=61; Stab_{V_0}=45=Spin(9,1)) AND re-pass the H^3 cone-Hessian sign benchmark K=−1/2, BEFORE any new structure-group count or curvature verdict.

### Phase A (→ Phase 75): Coframe-Reduction Dealbreaker — THE KILL GATE (do first; cheap)

- [x] **CALC-01**: Compute the EXACT image dimension over Q of `π_u(V_{1/2}(16))` under the C_u/Phase-46 bottleneck (expect 4). **KILL if ≠ 4.**
- [x] **CALC-02**: Compute the EXACT Gram-eigenvalue signature over Q of the induced coframe pairing (expect Lorentzian (1,3) / mostly-minus). **KILL if not Lorentzian.**
- [x] **VALD-02**: Decide (forced-vs-arbitrary) whether the residual structure group on the 4-dim coframe contains SO(3,1) and is FORCED by `(E_11,u)` alone, not by an arbitrary extra choice. **KILL (`fp-arbitrary-reduction`) if the 4d reduction requires an arbitrary choice.**

### Phase A.5 (→ Phase 76): Berry-Curvature Same-Wall Gate — SOFT KILL

- [x] **CALC-03**: Compute the QGT of the idempotent state family `|ψ(x)⟩` at `E(x)` (projector form `Q=Tr(P ∂P ∂P)`, gauge-invariant); its REAL part should be a sensible real symmetric metric of the SAME non-Einstein CHARACTER as the v17.0 cone-Hessian. **SOFT sanity check, NOT a hard stop:** `Re(QGT)` is the Fubini-Study pullback along `E(x)` — a DIFFERENT construction than the cone-Hessian restriction on the V_0 slice — so a discrepancy is INFORMATIVE (diagnose why), not a bug; do NOT KILL or HALT on this anchor alone.
- [x] **VALD-03**: Well-definedness checkpoint (survey-added) — establish that `Im(QGT) = F_B` is a well-defined, generically-NONZERO 2-form on the 4-dim slice AFTER the C_u breaking (OP^2=F_4/Spin(9) carries no invariant 2-form, so a nonzero Berry 2-form must be born from the breaking), BEFORE testing its shape.
- [x] **CALC-04**: Classify `F_B` at `M=0` — zero/flat, pure-Λ (`F_B ~ e∧e`), or other (report the vacuum level; expected flat/pure-Λ per CONVENTIONS §6; do NOT reintroduce Λ<0).
- [x] **VALD-04**: Same-wall test — with `M∈V_{1/2}` on, decide whether the matter-sourced `F_B` (and the ε-contraction of `F_B∧F_B` onto the Lorentz block) is transverse/Einstein-shaped vs EM-shaped (traceless `T^μ_μ=0`, `~F²`, conformal), matching M-power + tensor-structure + support against an independently-frozen `V_{1/2}` stress-energy `T[M]`; use gauge-invariant scalars (the degenerate-bundle Berry curvature is non-abelian/Wilczek-Zee, gauge-COVARIANT not invariant). **SOFT KILL if it reproduces the v17.0 cone-Hessian mismatch (support disjoint from T[M], no order-matching).**

### Phase B (→ Phase 77): Full Cartan Curvature = 4d Gravity (only if A and A.5 survive)

- [x] **DERV-03**: Confirm `e = π_u(dE)` is a non-degenerate soldering form on the 4-dim slice: `det(e^a_μ) ≠ 0` (exact over Q).
- [x] **DERV-04**: Extract `ω` = the Lorentz Spin(3,1) part of the ambient Spin(9,1) connection compatible with `e` (metric/torsion condition or the canonical f_4/e_6 reductive split `g=h⊕m`); do NOT use the raw 45-dim Spin(9,1) curvature.
- [x] **CALC-05**: Assemble `A = ω⊕e` (so(3,2)/so(4,1)/iso(3,1)) and compute `F = dA + A∧A` symbolically; identify the Lorentz block `R(ω)+Λe∧e` with the 4d Riemann tensor (cross-checked against an independent Totaro/Levi-Civita computation on ≥5 components, reusing `bulk_geometry_verification.py`); report the translation/torsion block `de+ω∧e`.
- [x] **VALD-05**: At `M=0`, test whether the Lorentz block is Einstein/(A)dS (`R_μν ~ Λg_μν`), sign-fixed by the K=−1/2 benchmark (expected flat/pure-Λ, MEASURE Λ); then turn on `M∈V_{1/2}` and characterize the matter-sourced Riemann.

### Phase C (→ Phase 78): Circularity Audit (the GST sin, re-armed)

- [x] **VALD-06**: Decide whether the MM ε-contraction (the Spin(9,1)→SO(3,1) breaking + ε-tensor that turns `F∧F` into Einstein-Hilbert+Λ) is FORCED by the h_3(O) trace-form/cubic-norm pairing — concretely, whether the space of trace-form-invariant quadratic-in-F contractions is **1-dimensional AND equals the ε-contraction with a cubic-norm-fixed normalization** (**STRONG WIN**) — or appears only because an MM/EH action was posited by hand (**`fp-imported-action`**, honest partial, NOT a derivation). Report at true strength; frame Singh/Castro/GST as the contrast class.
...
| ----------- | ----- | ---------------- |
| DERV-01 | Phase 74 (0: Engine & Calibration) | ✓ Complete — det SSOT re-passes (CH + 324/324 = dim f_4 52; octonion_algebra.py absent) |
| DERV-02 | Phase 74 (0: Engine & Calibration) | ✓ Complete — `E_11∘δ=½δ` exact over Q; `T_{E_11}OP²=V_{1/2}(16)` (kernel==span{11..26}) |
| VALD-01 | Phase 74 (0: Engine & Calibration) | ✓ Complete — calibration anchors (24/28/3; 78; 17; 61; 45) + K=−1/2 reproduced byte-for-byte |
| CALC-01 | Phase 75 (A: Coframe Reduction — KILL) | ✓ Complete — **SURVIVES**: dim π_u(V_{1/2}(16)) = 4 exact over Q (survivors {11,18,19,26}=C_u²) |
| CALC-02 | Phase 75 (A: Coframe Reduction — KILL) | ✓ Complete — **SURVIVES**: soldering-form metric signature (1,3), B rank 4 (trace-form (4,0) = Euclidean OP² foil) |
| VALD-02 | Phase 75 (A: Coframe Reduction — KILL) | ✓ Complete — **SURVIVES**: residual 21=so(3,1)[6]⊕so(6)[15]; SO(3,1) FORCED by (E_11,u); so(6) a trivial-on-spacetime ideal (fp-arbitrary-reduction averted) |
| CALC-03 | Phase 76 (A.5: Berry — RETIRED gate) | ✓ Complete (gate retired) — Re(QGT) sensible FS metric; SOFT sanity check only |
| VALD-03 | Phase 76 (A.5: Berry — RETIRED gate) | ✓ Complete (gate retired) — `Im(QGT)=F_B` well-defined, born-from-breaking, generically nonzero |
| CALC-04 | Phase 76 (A.5: Berry — RETIRED gate) | ✓ Complete (gate retired) — M=0 pure-Λ/Kähler vacuum `F_B=−2ω_K` |
| VALD-04 | Phase 76 (A.5: Berry — RETIRED gate) | ✓ Executed; SOFT-KILL **OVERTURNED** — `F_B` is the internal `so(6)=SU(4)` gauge curvature, NOT gravity (quadratic-in-F Maxwell-stress discriminant is a tautology that also kills real GR); deferred SM bonus |
| DERV-03 | Phase 77 (B: Full Cartan Curvature) | ✓ Complete — `e=π_u(dE)` invertible coframe, `det(e)=½≠0`, sig (1,3) (Lagrange-congruence fallback, frame-invariant) |
| DERV-04 | Phase 77 (B: Full Cartan Curvature) | ✓ Complete — closed-form torsion-free Levi-Civita `ω(e)`, sign-pinned K=−1/2 |
| CALC-05 | Phase 77 (B: Full Cartan Curvature) | ✓ Complete — `F=dA+A∧A` Lorentz block == independent Levi-Civita Riemann of g=e·e on 6/6 components exact over Q; torsion=0 |
| VALD-05 | Phase 77 (B: Full Cartan Curvature) | ✓ Complete — **NEGATIVE**: M=0 flat Λ=0 MEASURED; `G[g]` NOT Einstein-form `κT+Λg` for any single global (κ,Λ) vs AST-guarded order-matched `T[M]` (two-axis failure; t⁴ order-match satisfied → structural) |
| VALD-06 | Phase 78 (C: Circularity Audit) | ✓ Complete — **`fp-imported-action`**: ε-contraction NOT forced (dim-1 intrinsic subspace generated by Pontryagin not ε; ε only via orientation-ambiguous volume form; `det_3≡0` on the Lorentz block ⇒ normalization free); Λ=0 Gauss-Bonnet corollary; STRONG-WIN fails all 3 clauses |
...
_Last updated: 2026-06-03 — ARCHIVED at v18.0 completion (all 15 requirements complete; final verdict `fp-imported-action`)._
