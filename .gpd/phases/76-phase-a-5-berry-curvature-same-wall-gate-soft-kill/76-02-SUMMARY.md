...
> **⚠ DISPOSITION (2026-06-02): the PROPOSED SOFT KILL below was OVERTURNED by Bryan (NOT ratified).** A.5 tested the **wrong object**: the Maxwell stress / Pontryagin `F_B∧F_B` are *quadratic* in `F_B`, while the Einstein–Hilbert object `ε_{abcd}R^{ab}∧e^c∧e^d` is *linear* in `R[ω]`; and "a 2-form's Maxwell stress is traceless in 4d" is a universal tautology that also kills real GR. `F_B=Im(QGT)` is the **internal `so(6)=SU(4)` gauge curvature, NOT gravity** (deferred SM bonus). Gravity = the soldering-form Riemann `R[ω]`. A.5 does NOT gate gravity; **Phase 75 (SURVIVES) alone greenlights Phase B**, which now opens with a flatness sub-gate. The computed arithmetic below (43/43 PASS, exact over Q(i)) STANDS as a characterization of the internal-gauge sector; only the SOFT-KILL *verdict* is void. See `.gpd/ROADMAP.md` disposition banner, CONVENTIONS §11, and the decision log. The matter-half analysis below is retained for the record.
