...
The current continuation architecture separates execution provenance from bounded-resume authority. The portable execution story now has an append-only lineage record plus a derived execution head, while `gpd init resume` computes the canonical continuation view by reading `state.json.continuation` first and only consulting compatibility surfaces when the canonical bounded segment is missing or incomplete. The canonical public resume surface centers on `active_resume_kind`, `active_resume_origin`, `active_resume_pointer`, `active_bounded_segment`, `derived_execution_head`, `continuity_handoff_file`, `recorded_continuity_handoff_file`, `missing_continuity_handoff_file`, and `resume_candidates`. The canonical fields above are the public resume surface. Legacy aliases are nested compatibility-only cues inside `segment_candidates` and the legacy `session` mirror; they are recovery aids, not top-level public fields. `session`, `.continue-here.md`, and `DERIVATION-STATE.md` are supporting projections only; they may mirror canonical continuation or provenance, but they do not compete with it:
