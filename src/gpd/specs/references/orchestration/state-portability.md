...
4. **Resume**: Run `/gpd:resume-work` in the coding assistant when you are ready to continue work there, or `gpd resume` from your normal system terminal for a read-only local recovery summary. The shared resume machinery reads `GPD/state.json` and reconstructs the full project context.
