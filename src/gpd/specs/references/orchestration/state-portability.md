...
- **Task level**: bounded execution state, active resume pointer, interrupted agents, and live review gates (the raw compatibility/debug block may still expose nested cues such as `compat_resume_surface.current_execution`, `compat_resume_surface.execution_resume_file`, and `compat_resume_surface.segment_candidates`)
...
2. **Resume artifacts**: Canonical `state.json.continuation.bounded_segment` is the primary durable bounded-segment state. A derived execution head contributes a bounded-segment candidate only when that canonical field is absent and its `resume_file` still resolves to a portable repo-local path. When the legacy `session` handoff mirror resolves to an existing repo-local file, it surfaces as a non-resumable recorded handoff candidate, and it becomes `continuity_handoff_file` / `active_resume_pointer` only as the fallback pointer when no usable derived execution head exists. If the recorded handoff is repo-local but missing, resume surfaces a missing recorded handoff artifact plus an advisory recorded handoff candidate with `status: "missing"`. In the raw compatibility/debug block those cues can still appear at `compat_resume_surface.session_resume_file` and `compat_resume_surface.missing_session_resume_file`. A derived head without a portable usable resume file remains advisory continuity context only.
...
The resume workflow ranks three semantic recovery families into `resume_candidates`: bounded execution snapshot, recorded handoff, and interrupted agent. In the raw compatibility/debug block those families can still appear inside `compat_resume_surface.segment_candidates`, whose legacy labels may include `current_execution`, `session_resume_file`, and `interrupted_agent`. The recorded handoff source can appear either as a non-resumable `handoff` candidate or as an advisory `missing` candidate. It does not synthesize additional candidates from auto-checkpoints, filesystem drift, or machine-change state. A `.continue-here.md` file by itself is only a temporary continuation handoff artifact; it becomes part of the canonical continuation view only when reached through the ranked runtime surfaces. `compat_resume_surface.current_execution` remains a compatibility fallback cue whenever canonical continuation is missing or incomplete; when canonical `state.json.continuation` is complete, it stays the source of truth first.
