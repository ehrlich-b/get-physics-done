...
- **Task level**: bounded execution state, active resume pointer, interrupted agents, and live review gates (the current raw envelope still exposes compatibility labels such as `current_execution`, `execution_resume_file`, and `segment_candidates`; the legacy candidate names stay nested compatibility-only cues)
