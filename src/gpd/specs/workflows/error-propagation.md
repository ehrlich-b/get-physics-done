...
If the target is not found in `STATE.md` `intermediate_results`, first use `gpd result search` to look for the canonical result by identifier, equation, or description. Once you have the canonical `result_id`, prefer `gpd result deps "{result_id}"` to recover the recorded dependency tree before reconstructing it manually. If the target still is not found, check SUMMARY.md files across phases for the quantity name.
</step>
