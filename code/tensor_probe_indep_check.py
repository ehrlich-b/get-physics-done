...
def general_york_solve(target_blocks, D, kw=2, kf=2, g=None, ginv=None, Gam=None, GamB=None,
    ...
    Returns (consistent, n_unknowns, n_eqs, residual_rank_info).
    ...
    linsolve over Q.  If points_mode, also do a redundant point-eval rank check."""
    if g is None:
        g = metric()
    if ginv is None:
        ginv = metric_inv(g)
    ...
    nm = len(monos)
    rho = _rho()
    # 4 omega components + 1 conformal f  => 5*nm unknowns
    ...
    for k in range(3):
        for a in range(2):
            for b in range(2):
                ...
                num = expand(num)
                # num is LINEAR in the unknowns allc; match every (z,zbar)-monomial coeff to 0.
    ...
    return consistent, len(allc), len(eqs), None
