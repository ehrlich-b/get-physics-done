...
# ASSERT_CONVENTION: natural_units=natural, metric_signature=riemannian, fourier_convention=NA, gauge_choice=NA, renormalization_scheme=NA, coupling_convention=c=Tr(XoY); jordan=(1/2)(AB+BA); fano e1e2=e4; c(X,X)=Tr(X^2) (NOT (Tr X)^2); 27=1(+)26 (trivial Tr-direction (+) trace-free 26); weight multiset {0^3} U {24 short roots = perms(+/-1,+/-1,0,0) norm^2=2}; 48 roots = 24 short + 24 long(+/-e_i,(+/-1/2)^4 norm^2=1); |W(F_4)|=1152; gen_func vars s=X-degree t=Y-degree H(s,t) bigraded; arithmetic=exact-SymPy-over-Q; ranks/nullspace=exact_qq_rank=DomainMatrix-over-QQ (NEVER numpy.linalg.matrix_rank / SVD tolerance); Molien CT=symbolic iterated residue (NEVER numerical torus grid); Krull target=10 (the SUPERSEDED 7 is FORBIDDEN)
# REP-DECOMP: 27 = 1 (trivial/Tr direction) (+) 26 (trace-free irreducible, self-dual); 26's nonzero weights = the 24 short roots of F_4
