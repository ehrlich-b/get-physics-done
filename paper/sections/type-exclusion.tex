...
\section{Type Exclusion and C*-Algebra Promotion}\label{sec:type-exclusion}
...
classification~\cite{JvNW1934} asserts that every finite-dimensional
...
Wilce~\cite{BGW2020} prove that no non-signaling composite with product
states exists for any EJA containing an Albert algebra summand
(Theorem~1 of~\cite{BGW2020}). The 27-dimensional exceptional Jordan
...
\begin{theorem}[{C*-algebra structure;
  \cite[Theorem~3]{vdW2019}}]\label{thm:vdW3}
...
\midrule
$V$ satisfies S1--S7
  & Verified & Section~\ref{sec:axiom-verification} \\
...
Let $V$ be a finite-dimensional EJA admitting a locally tomographic
composite and containing a qubit subsystem ($V_3 \cong
M_2(\mathbb{C})^{\mathrm{sa}}$). Then $V \cong
M_n(\mathbb{C})^{\mathrm{sa}}$ for some $n \ge 1$.
\end{theorem}
...
$V$ admits a locally tomographic composite
  & Verified & Theorem~\ref{thm:local-tomo} \\
$V$ contains a qubit subsystem $V_3$
  & Verified & EJA face structure
  (any orthogonal pair $\{p,\,p^\perp\}$ \\
  & & defines a $V_3$-like 2-level sector) \\
\bottomrule
\end{tabular}
\end{center}
...
Let $V$ be a JB-algebra. If $V$ admits a tensor product (in the sense
that $V\otimes V$ is again a JB-algebra with product states), then $V$ is
the self-adjoint part of a C*-algebra.
\end{theorem}

\noindent
The hypotheses of Theorem~\ref{thm:HO} are satisfied: $V$ is a
finite-dimensional EJA (hence a JB-algebra), and the composite $V_{BM}$
with S1--S7 provides the required tensor product structure. This theorem
is consistent with Theorems~\ref{thm:vdW3} and~\ref{thm:BW} and
provides an independent route to the C*-algebra conclusion.
...
\item The self-modeling constraint yields a sequential product on
  ...
  semantics (Section~\ref{sec:construction}).
\item The corrected product formula~\eqref{eq:corrected-product},
  ...
  (Section~\ref{sec:axiom-verification}).
