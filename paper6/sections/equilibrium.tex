...
% ASSERT_CONVENTION: natural_units=natural, metric_signature=mostly_minus, entropy_base=nats, modular_hamiltonian=K_minus_ln_rho, coupling_convention=H_sum_hxy, state_normalization=Tr_rho_1
...
CFT~\cite{Affleck1988}.  In these conformal theories, the modular
...
($d = 1$).  For $d \geq 2$, the infrared theory of the $SU(n)$
Heisenberg model is not a free CFT, and the conformal approximation
discussed in Sec.~\ref{sec:conformal-approx} applies in a window
$a \ll R \ll 1/m$ (if such a window exists).

\subsection{Wilsonian continuum limit}
\label{sec:continuum}

At length scales $L \gg a$ (lattice spacing), the lattice details
become irrelevant by the standard Wilsonian universality argument.
The long-wavelength physics is governed by the symmetry-allowed
effective field theory.  The self-modeling lattice provides the UV
completion; the smooth Riemannian manifold $(M, \gab)$ emerges at
long wavelengths.

This is the same assumption made in lattice QCD, causal dynamical
triangulations, and Regge calculus: the passage from a discrete
structure to a smooth manifold at long wavelengths is a universal
feature of lattice approaches to quantum gravity, not specific to
self-modeling.  A rigorous constructive proof of this continuum limit
(showing that lattice correlation functions converge to those of a
local quantum field theory on a Riemannian manifold) is a hard open
problem shared by all lattice quantum gravity programs.  We frame the
continuum limit as a physical argument, not a rigorous construction.
