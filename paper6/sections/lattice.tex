...
Ref.~\cite{vandeWetering2018}, and it inherits $U(n)$ covariance from
...
compatible with the algebraic structure; the consequences for
entanglement are analyzed in Sec.~\ref{sec:arealaw}.
...
For the self-modeling Hamiltonian on the one-dimensional lattice
$\mathbb{Z}^1$ (coordination number $z = 2$), using the
Nachtergaele--Sims framework~\cite{NachtergaeleSims2006} with
exponential decay function $F_a(r) = e^{-ar}$ at $a = 1$:
\begin{equation}
  \vlr = \frac{8eJ}{e - 1} \approx 12.66\, J.
  \label{eq:vlr}
\end{equation}
Crucially, $\vlr$ is \emph{independent of the local dimension~$n$}:
the SWAP operator $F$ satisfies $\|F\| = 1$ for all $n \geq 2$, so
the Lieb--Robinson velocity depends only on the coupling strength~$J$
and the lattice coordination number~$z$.
