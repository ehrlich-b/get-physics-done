...
rotations at each site).  The coupling, however, is constrained to
\emph{diagonal} $U(n)$ by the self-modeling structure, as follows.

The self-model $M$ interacts with both sites $x$ and $y$ through the
shared boundary.  The model has \emph{one} state space
$M_n(\mathbb{C})^{sa}$ and one $U(n)$ covariance group: it describes
both sites using the same algebraic labels.  A basis relabeling
$U \in U(n)$ of the model's description acts identically on both
sites---$M$ has no independent reference frame for $x$ and $y$
separately.  This is the physical content of self-modeling at a
boundary: the model probes both sides through the \emph{same} sequential
product structure, so rotations act as $U \otimes U$ (diagonal), not as
$U_x \otimes U_y$ (independent).  Full $U(n) \times U(n)$ invariance
would require two independent probes with independent reference frames,
contradicting the single-subsystem structure of the self-model.

This constrains $h_{xy}$ to be invariant under the diagonal $U(n)$:
\begin{equation}
  (U \otimes U)\, h_{xy}\, (U \otimes U)^\dagger = h_{xy}
  \qquad \forall\, U \in U(n).
  \label{eq:diagonal-invariance}
\end{equation}
Note that full $U(n) \times U(n)$ invariance would kill all coupling,
leaving $h_{xy} \propto \1 \otimes \1$.  Diagonal $U(n)$ is the maximal
symmetry compatible with nontrivial interaction.
