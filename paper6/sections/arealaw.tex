...
In all cases, the entanglement entropy of the self-modeling lattice
ground state scales at most as
\begin{equation}
  \ent(A) \lesssim \log(n) \cdot |\partial A|,
  \label{eq:channel-bound}
\end{equation}
where $\log n$ is the maximum entanglement per boundary bond (from the
Schmidt decomposition of a single $\mathbb{C}^n \otimes \mathbb{C}^n$
bond).  This is an upper bound, saturated by maximally entangled bonds;
actual ground-state entanglement is typically smaller.
...
\item \textbf{Ground states:} $\ent(A) \lesssim \log(n)\,|\partial A|$
  (Heisenberg model: Calabrese--Cardy in 1D, Hastings for gapped,
  numerical evidence in 2D).
