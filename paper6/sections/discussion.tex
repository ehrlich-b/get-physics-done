...
\emph{Standard physics: Wilsonian continuum limit.}---The passage from a
lattice theory to a smooth Riemannian manifold at long wavelengths
$L \gg a$ (where $a$ is the lattice spacing) is a standard Wilsonian
universality argument.  At long wavelengths the lattice details are
irrelevant and the physics is governed by the symmetry-allowed effective
field theory.  This is the same assumption made in lattice QCD, causal
dynamical triangulations, and Regge calculus---it is a universal feature
of lattice approaches to quantum gravity, not specific to self-modeling.
...
nonconformal fields, corrections of order $O\!\left((mR)^{2\Delta}\right)$
appear~\cite{Speranza2016}, where $m$ is the mass scale breaking
conformal invariance and $R$ is the ball radius.  These corrections modify
the derived Einstein equation at subleading order.  In the window
$a \ll R \ll 1/m$ (if it exists), the corrections are suppressed.  For
