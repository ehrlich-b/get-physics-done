...
$B$ be a geodesic ball of proper radius $R$ centered at $p$ in the
spacelike hypersurface $\Sigma = \{x^0 = 0\}$, with $a \ll R \ll
...
limit ($R \ll L_{\mathrm{curv}}$) is
\begin{equation}
  \delta\mathcal{A}
  = -\frac{\Omega_{d-1}\, R^{d+1}}{2d}
  \left[(d+1)\, \Rab\, n^a n^b + R\right]
  + O(R^{d+3}),
  \label{eq:delta-area}
\end{equation}
where $\Omega_{d-1} = 2\pi^{d/2}/\Gamma(d/2)$ is the area of the unit
$(d{-}1)$-sphere, $n^a$ is the unit timelike normal to $\Sigma$, and
$R = g^{ab}\Rab$ is the Ricci scalar.

The UV entropy variation is therefore
\begin{equation}
  \delta\ent_{\mathrm{UV}} = \eta\,\delta\mathcal{A}
  = -\frac{\eta\,\Omega_{d-1}\, R^{d+1}}{2d}
  \left[(d+1)\,\Rab\, n^a n^b + R\right].
  \label{eq:ds-uv}
\end{equation}
For positive energy density ($\Rab\, n^a n^b > 0$ via the null energy
condition), $\delta\ent_{\mathrm{UV}} < 0$: positive energy causes
focusing, which decreases the area and hence the UV entropy.
...
$\delta\ent_{\mathrm{mat}} = \delta\langle\modH_B\rangle$.  For the
vacuum state of a conformal field theory restricted to a geodesic ball
$B$ of radius $R$, the Casini--Huerta--Myers (CHM) modular Hamiltonian
takes the exact form~\cite{CasiniHuertaMyers2011}
\begin{equation}
  \modH_B = 2\pi \int_B d^d\!x\; \frac{R^2 - |\mathbf{x}|^2}{2R}\; T_{00}(\mathbf{x}).
  \label{eq:chm}
\end{equation}
...
\begin{equation}
  ...
  = \frac{2\pi\, \Omega_{d-1}}{d(d+2)}\, R^{d+1}\, \Tab\, n^a n^b.
  \label{eq:ds-mat}
\end{equation}
...
corrections of order $O\!\left((mR)^{2\Delta}\right)$ appear, where
$m$ is the mass scale breaking conformal invariance and $\Delta$ is
its scaling dimension~\cite{Speranza2016}.  In the window
$a \ll R \ll 1/m$ (if it exists), these corrections are suppressed.
...
cancelling the common factor $\Omega_{d-1}\, R^{d+1}/d > 0$:
\begin{equation}
  \frac{\eta(d+1)}{2}\, \Rab\, n^a n^b + \frac{\eta\, R}{2}
...
the Einstein tensor $\Gab = \Rab - \frac{1}{2} R\, \gab$, and
...
\emph{Sign chain.}---The derivation produces attractive gravity via
the following sign chain:
\begin{enumerate}
\item Positive mass: $\Tab\, n^a n^b > 0$ (positive energy density).
