...
\begin{equation}
  \modH_B = 2\pi \int_B d^d\!x\; \frac{\ell^2 - |\mathbf{x}|^2}{2\ell}\; T_{00}(\mathbf{x}).
  \label{eq:chm}
\end{equation}
...
\item \emph{Entanglement first law.}  Perturbations of the entanglement
  ...
  emergent geometry.  The resulting equation relating metric
  perturbations to matter perturbations is therefore a symmetric,
  divergence-free 2-tensor equation built from the metric and its
  derivatives (because both sides are geometric quantities).
...
symmetric, divergence-free 2-tensor equation in the emergent
$(d{+}1)$-dimensional spacetime, Lovelock's theorem forces it to be
\begin{equation}
  \Gab + \Lambda\, \gab = 8\pi\Gnewton\, \Tab,
  \label{eq:einstein-lovelock}
\end{equation}
the same Einstein equation as Route~A.  The proportionality constant
$\Gnewton = 1/(4\eta)$ is fixed by matching the UV entanglement
density, as in Sec.~\ref{sec:parameters}.
...
In $d = 1$, Route~A gives the (trivially satisfied) Einstein equation
$\Gab = 0$ via the exact WZW modular Hamiltonian, while Lovelock's
theorem provides no constraint (in $1{+}1$ dimensions, every metric
satisfies $\Gab = 0$).  In $d \geq 2$, Route~B forces the Einstein
equation by uniqueness without requiring conformal symmetry of the
infrared theory, while Route~A provides the explicit computation in the
conformal window $a \ll \ell \ll 1/m$.  Together, the two routes cover
all dimensions.
